...
void UWFC::WFC(const int32 width,const int32 height)
{
	...
	while (true)
	{
		...
		if (AC3() == false)
		{
			UE_LOG(LogClass, Log, TEXT("---------------- DEU MERDA BACKTRACKING -----------------"));
		}
...
EObserveStatus UWFC::Observe()
{
	...
	if (tile != nullptr)
	{
		...
		if (tile.GetDefaultObject()->StaticMesh != nullptr)
		{
			UE_LOG(LogClass, Log, TEXT("veio null essa merda"));
		}
    			...
    			if (MBool[x][y] == false)
    			{
    				if (x + Directions_x[i] >= 0 && x + Directions_x[i] < Width && y + Directions_y[i] >= 0 && y + Directions_y[i] < Height && MBool[x + Directions_x[i]][y + Directions_y[i]] == true)
    				{
    					...
    					for (auto it = Grid[x+Directions_x[i]][y+Directions_y[i]]->ValidTiles.CreateConstIterator(); it; ++it)
    					{
    						if (i == 0)
    						{
    							if (!Grid[x][y]->ValidNeighboursS.Find(it.Key()))
    							{
    						...
    						if (i == 1)
    						{
    							if (!Grid[x][y]->ValidNeighboursN.Find(it.Key()))
    							{
    								TilesToRemove.Add(it.Key());
    							}
    						...
    						if (i == 2)
    						{
    							if (!Grid[x][y]->ValidNeighboursE.Find(it.Key()))
    							{
    								TilesToRemove.Add(it.Key());
    							}
    						...
    						if (i == 3)
    						{
    							if (!Grid[x][y]->ValidNeighboursW.Find(it.Key()))
    							{
    								TilesToRemove.Add(it.Key());
    							}
			    	...
			    	if (x + Directions_x[i] >= 0 && x + Directions_x[i] < Width && y + Directions_y[i] >= 0 && y + Directions_y[i] < Height && MBool[x + Directions_x[i]][y + Directions_y[i]] == true)
			    	{
			    		...
			    		for (auto it = Grid[x+Directions_x[i]][y+Directions_y[i]]->ValidTiles.CreateConstIterator(); it; ++it)
			    		{
			    			...
			    			for (auto it2 = Grid[x][y]->ValidTiles.CreateConstIterator(); it2; ++it2)
			    			{
								...
								if (i == 0)
								{
									if (!it2.Value().GetDefaultObject()->ValidNeighboursS.Find(it.Key()))
									{
			    				...
			    				if (i == 1)
			    				{
			    					if (!it2.Value().GetDefaultObject()->ValidNeighboursN.Find(it.Key()))
			    					{
			    				...
			    				if (i == 2)
			    				{
			    					if (!it2.Value().GetDefaultObject()->ValidNeighboursE.Find(it.Key()))
			    					{
			    				...
			    				if (i == 3)
			    				{
			    					if (!it2.Value().GetDefaultObject()->ValidNeighboursW.Find(it.Key()))
			    					{
...
TSubclassOf<UObjectTile> UWFC::GetRandomTile()
{
	...
	UE_LOG(LogClass, Log, TEXT("Random Tile: x:%d, y:%d, rand:%d, numTiles: %d"), x, y, rand, Grid[i][y]->ValidTiles.Num());
